...
frame
	...
	if(targ=="action") {(function(){
var scroll = frame.find('.browser_scroll').textScroll();
...
advantages_images.forEach(function(e){
    e.src.forEach(function(src, i) {
        images_array.push({
            src: src,
            title: e.title,
            show_text: ((i == 0) ? true : false)
        })
    })
})
...
frame
    ...
    .on('click', '.advantages__item-image', function() {
        ...
        if($(this).parents('.carousel_item').hasClass('active')) {
            ...
            var start_image = $(this).data('targ');
            var new_start_image

            images_array.forEach(function(e, i) {
                if(e.src == image){
                    new_start_image = i;
                }
            })
...
test_json('living', function() {
    ...
    $.each(data.living.apartments, function(k, v) {
        min_tc = min_tc > v.tc ? v.tc : min_tc;
        max_tc = max_tc < v.tc ? v.tc : max_tc;
    });

    mortgage_calculator = frame.find('.calc_bg').mortgageCalculator({
        cost: {
            min: min_tc,
            max: max_tc,
            current: flat_id && data.living.apartments[flat_id] ? data.living.apartments[flat_id].tc : 9000000,
            step: 50000,
            round: 1
        },
		initial_payment: {
    ...
    if(flat_id && data.living.apartments[flat_id]) {
        var d = data.living.apartments[flat_id];
        frame.find('.calc_close').data('targ', '/plans/korpus' + d.b + '/section' + d.s + '/floor' + d.f + '/flat' + d.n);
    }
...
function init_content() {
    ...
    slideshow = frame2.find('.home__slideshow-image').slideshowInit({
        ...
        beforeChange: function (img, cur_number, old_number, direction) {
            ...
            frame2
                .find('.commercial-description__info-block')
                .transitionStop(true).transition({'opacity' : 0, 'y' : '-100%'}, 800, function() {
                    $(this).find('.home__tile-text').html(gallery_images[cur_number][1]);
                })
                .delay(200).transition({'opacity' : 1, 'y' : 0}, 800);
...
var $minimap_bubble = frame.find('.minimap_bubble');

plans_current_values.type = 'commercial';
plans_current_values.sect_sel = floor_frame.find('.plans__section-label');
...
test_json('commercial', function() {
    ...
    frame.find('.plans__korpus-bubble').each(function() {
        ...
        $bubble.html('' +
            '<div class="label">' +
                '<div>' +
                    '<div class="flats-count"></div>' +
                '</div>' +
                (window.start_buildings[korp_num] ? '<div class="start"><span>Старт продаж</span></div>' : '') +
            '</div>' +
            '<div class="plans__korpus-bubble-num"><span>' + get_korp_name_short(korp_num) + '</span></div>'
        )
            .toggleClass('soon', window.soon_buildings[korp_num])
            .toggleClass('sold', window.sold_buildings[korp_num]);
    });
    ...
    function filter_buildings(result) {
        ...
        frame.find('.plans__korpus-bubble').each(function() {
            ...
            var soon = soon_buildings[dt];
            if(sold_buildings[dt]) {
                $bubble.addClass('active').find('.flats-count').text('Продано');
            } else {
                $bubble.toggleClass('active', !!count || soon).find('.flats-count').html(soon ? 'Скоро <br />в продаже' : 'Найдено: ' + count);
            }
            if(!soon){
    ...
    function filter_floors(result) {
        ...
        for(var i = 0, l = result.length; i < l; i ++) {
            ...
            if(d.b === plans_current_values.b) {
				...
				var idx = plans_current_values.b+"-"+d.s;
                if (aItems.indexOf(idx)===-1){
                    aItems.push(idx);
                }
            }
        }
    ...
    function filter_aparts(result) {
        ...
        svg_paper_floor.forEach(function(el) {
            ...
            if(($.inArray(alt, result) + 1) && el.st) {
                if(el.st > 1 || el.st == 0){
                    el.bottom.attr({'opacity' : 0.4, 'fill' : '#6d777b', 'fill-opacity' :0.5});
                } else {
        ...
        svg_paper_korp = korpus_frame.find('.plans_map_cont').area2svg({
            ...
            mouseover: function (el) {
                ...
                if(el.at) {
                    ...
                    floor_popup
                        .addClass('active').css(box.x2 * 100 / svg_paper_korp.width < 70 ? {'top' : el.getSideCenters().ry * scale, 'left' : box.x2 * scale, 'margin-left' : 0} : {'top' : el.getSideCenters().ly * scale, 'left' : box.x * scale, 'margin-left' : '-27em'})
                        .find('> .center .n2').text(spl[1] > 9 ? spl[1] : '0' + spl[1]).end()
                    ...
                    each: function (el) {
                        ...
                        if (!d || d.st != 1) {
                            if (!d) {
                             ...
                             el.bottom.attr({'opacity' : 0.19, 'fill' : '#6d777b', 'fill-opacity' :0.5});
                        } else {
                            el.bottom.attr({'opacity' : 0.19, 'fill' : '#c1d72e', 'fill-opacity' : i % 2 ? 0.5 : 1});
        ...
        svg_paper_quarter_minimap = frame.find('.quarter_map_cont').area2svg({
            ...
            click: function (el) {
                if(el.at && !el.active) {
                    ...
                    if(spl[0] == plans_current_values.b) {
                        load_floor_animation({
                            section: spl[1]
                        });
                    } else {
                        pjax.loadPage('/commercial/korpus' + spl[0] + '/section' + spl[1] + '/floor' + plans_current_values.f);
                    }
                }
            ...
            mouseover: function (el) {
                if(el.at && !el.active) {
                    ...
                    el.bottom.stop().animate(200).attr({'opacity' : 0.6,'fill': '#009fe3'});

                    $minimap_bubble
                        .find('.val').text(get_korp_name_short(spl[0]) + ', секция ' + spl[1]).end()
                        .addClass('visible');
                }
            },
            mouseout: function (el) {
                if(el.at && !el.active) {
                    el.bottom.stop().animate(200).attr({'opacity' : 0.2, 'fill' : '#c8d400'});
                }

                $minimap_bubble.removeClass('visible');
            },
            each: function (el) {
    ...
    function update_quarter_minimap() {
        svg_paper_quarter_minimap.forEach(function(el) {
            if(el.alt === plans_current_values.b + '-' + plans_current_values.s) {
                el.bottom.attr({'opacity' : 1,'fill': '#009fe3'});
                el.attr({'cursor' : 'default'});
                el.active = 1;
            }
            else if(data.commercial.sections[el.alt] && data.commercial.sections[el.alt].at) {
                el.bottom.attr({'opacity' : 0.4, 'fill' : '#c8d400'});
                el.attr({'cursor' : 'pointer'});
                el.active = 0;
                el.at = data.commercial.sections[el.alt].at;
            }
            else {
    ...
    function load_floor_details() {
        ...
        test_next_btn(plans_current_values.sect_sel.find('.section_prev'), get_closest_section(plans_current_values.s, -1));
        test_next_btn(plans_current_values.sect_sel.find('.section_next'), get_closest_section(plans_current_values.s, 1));
        ...
        plans_current_values.sect_sel.find('.n2').text(plans_current_values.s > 9 ? plans_current_values.s : '0' + plans_current_values.s);
    ...
    frame
        ...
        .on('mouseleave', '.plans__korpus-bubble', function() {
            if($(this).hasClass('active')) {
                svg_paper_bg.getByAlt($(this).data('targ'))[0].stop(true).animate(100).attr({'fill' : '#009fe3'});
            }
        })
        ...
        .on('click', '.section_prev.active, .section_next.active', function() {
            load_floor_animation({
...
frame
    ...
    .on('click', '.plans__offers-close', function() {
     ...
     .on('click', '.plans__filters-toggle', function() {
        frame.toggleClass('opened_filters');
    });

frame
    .on('mousemove', function(e) {
        $minimap_bubble.css({'top' : e.pageY, 'left' : e.pageX});
    });
	}())}
...
var slideshow = frame2.find('.about__gallery').slideshowInit({
...
frame
	...
	if(targ=="construction-video") {(function(){
page.unset=function(){
    frame.off();
    unloadPlugin(carousel);
};
...
ymaps.ready(function() {
    ...
    map.geoObjects.add(new ymaps.Placemark(obj_point, {
            ...
            iconImageClipRect: [[789, 528], [971, 601]],
            iconImageHref: '/assets/i/sprite.png',
            iconImageSize: [182, 73],
    ...
    map.geoObjects.add(new ymaps.Rectangle(
        ...
        {
            stroke: false,
            fillImageHref: '/assets/i/map_top_contacts.png'
        }
    ));
    ...
    var control = new ymaps.control.RoutePanel({
      ...
      var controlManager = new ymaps.control.Manager(map);
    frame
...
frame
    .on('click', '.home__tile', function(){
        $(this).siblings('.home__tile.active').removeClass('active').end().toggleClass('active');
    })
	}())}
	if(targ=="design2") {(function(){
page.unset=function(){
    frame.off();
    unloadPlugin(slideshow);
    unloadPlugin(scroll_controller);
...
if(pages_info.previous && pages_info.previous.type !== 'gallery'){
    frame.find('.page__header')
        .css({'opacity' : 0, 'y' : '-100%'})
        .delay(800).transition({'opacity' : 1, 'y' : 0}, 800);
    frame.find('.about__submenu')
...
function animate_layout(delay){
    ...
    frame.find('.home__button').addClass('hidden').each(function(i) {
        var $that = $(this);

        $that.delay(delay + 2700 + (frame.find('.home__button').length - 300 * i)).queue(function(next) {
...
frame
    ...
    .on('click', '.filter_buttons', function() {
        frame.find('.filter_buttons').toggleClass('active');
        frame.find('.news_item').toggleClass('hidden');
    })
...
test_json('parking', function() {
    ...
    function change_stage(opt) {
        ...
        switch (stage) {
            ...
            case 1:
                ...
                load_korpus(change_stage_details);
    ...
    function filter_buildings(result) {
        ...
        frame.find('.plans__korpus-bubble').each(function() {
            ...
            if (!count) {
                    $bubble.addClass('active sold').find('.flats-count').text('Продано');
            } else {
            ...
            svg_paper_bg.getByAlt(dt)[0].stop(true).animate(300).attr({'opacity' : count? 0.33 : 0, 'cursor' : count?'pointer' : 'default'});
            svg_paper_bg.getByAlt(dt)[0].data('active', count? 1 : 0);
        });
    }
    ...
    function load_korpus(callback) {
     ...
     function load_korpus_map() {
        ...
        svg_paper_korp = korpus_frame.find('.plans_map_cont').area2svg({
            ...
            'height' : 1080,
            click: function (el) {
                var alt = plans_current_values.b + '-' + el.alt;
                if(d[alt].at){
                    var spl = el.alt.split('-');
                    plans_current_values.s = Number(spl[0]);
                    plans_current_values.f = Number(spl[1]);
...
*/
                    floor_popup
                        .addClass('active').css(box.x2 * 100 / svg_paper_korp.width < 70 ? {'top' : el.getSideCenters().ry * scale, 'left' : box.x2 * scale, 'margin-left' : 0} : {'top' : el.getSideCenters().ly * scale, 'left' : box.x * scale, 'margin-left' : '-15em'})
                        .find('> .center .n2').text(spl[1] > 9 ? spl[1] : '0' + spl[1]).end()
                        .find('.plans__floor-label-at .n2').text(d[alt].at);
                }
            },
            mouseout: function (el) {
                var alt = plans_current_values.b + '-' + el.alt;
            ...
            each: function (el) {
                var alt = plans_current_values.b + '-' + el.alt;
                if(d[alt].at){
                    el.attr({'fill': '#009fe3', 'opacity' : 0.33});
                }
                //el.attr({'fill': '#009fe3'});
            }
        });
            ...
            fr.data('targ', floor_id).find('.plan_draggable').load('/hydra/park/floors/' + floor_id + '.html', function() {
                ...
                svg_paper_floor = fr.find('.floor_map_cont').area2svg({
                    ...
                    click: function (el) {
                        ...
                        change_stage({'stage' : el.selected? 3 : 2});
                    },
                    mouseover: function (el) {
    ...
    function update_apart_state(el){
        ...
        if(el.selected){
            ...
            if(d.full_type == 'Место хранения' ){//&& d.b!=33 && d.b!=34
                attrs.fill ='#fad719';    
            }
            attrs.cursor = 'pointer';
        } else {
    ...
    function load_floor_details() {
        ...
        plans_current_values.floor_sel.find('.n2').text(((plans_current_values.b!=40) ? '-' : '') + plans_current_values.f);
        ...
        if(plans_current_values.b==40 ){//|| plans_current_values.b==33 || plans_current_values.b==34
            frame.find('.legend_item.moto').hide();
        } else {
            frame.find('.legend_item.moto').show();
        }
    }
    ...
    frame
        ...
        .on('click', '.plans__back', function(){
            ...
            if(url){
                ...
                if(plans_current_values.b != 40 && frame.find('.plans__frame').hasClass('opened_floor')){
                    change_stage({'delta' : -2});
                } else {
                    change_stage({'delta' : -1});
                }
            }
...
test_json('living', function() {
    ...
    $.each(data.living.apartments, function(id, apart){
        if (rooms.indexOf(apart.rc) === -1 && apart.st == 1) rooms.push(apart.rc);
    });

    for (var i = 0; i <= 4; i++) {
        if (rooms.indexOf(i) === -1) {
            frame.find('.search_buttons.rc[data-targ="'+i+'"]').addClass('hidden');
        } 
    }
    svg_paper_bg = plans_frame.find('.plans_map_cont').area2svg({
        'opacity': 0,
        ...
        click: function (el) {
            if (el.data('active')) {
                ...
                korpus_reverse = false;
                // if(el.alt == '32'){
                //     pjax.loadPage('/plans/search?b=32');
                // } else {
                    change_stage({'stage' : 1});
                //}
            }
    ...
    */
    ...
    frame.find('.plans__korpus-bubble').each(function() {
        ...
        $bubble.html('' +
            ...
            '<div>' +
            '<div class="flats-count"></div>' +
            '<div class="release-date">' + (release_date ? release_date : '') + '</div>' +
            '</div>' +
            (window.start_buildings[korp_num] ? '<div class="start"><span>Старт продаж</span></div>' : '') +
            (window.design_buildings[korp_num] ? '<div class="design"><span>Есть с отделкой</span></div>' : '') +
            '</div>' +
            '<div class="plans__korpus-bubble-num"><span>' + get_korp_name_short(korp_num) + '</span></div>'
        )
            .toggleClass('soon', window.soon_buildings[korp_num])
        ...
        if(korp_num == 'p') $bubble.addClass('parking');
    });
    //frame.find('.points_drag').dragChildrens();
    ...
    function change_url() {
        ...
        switch (stage) {
            case 1:
                url += '/korpus' + plans_current_values.b + ((utm_param) ? utm_param : '');
                break;
            case 2:
                url += '/korpus' + plans_current_values.b + '/section' + plans_current_values.s + '/floor' + plans_current_values.f + ((utm_param) ? utm_param : '');
                break;
            case 3:
                url += '/korpus' + plans_current_values.b + '/section' + plans_current_values.s + '/floor' + plans_current_values.f + '/flat' + data.living.apartments[plans_current_values.n].n + ((utm_param) ? utm_param : '');
            default:
                break;
        }
        pjax.loadPage(url + (encode_string ? '?' + encode_string : ''), {'suppress_load' : true});
    ...
    function filter_buildings(result) {
        ...
        frame.find('.plans__korpus-bubble').each(function() {
            ...
            var count = active_bs[dt] || 0;
            var soon = soon_buildings[dt];
            if (!sold_buildings[dt] && !count && !soon) {
                $bubble.addClass('active').find('.flats-count').text('Найдено: 0');
            } else
            if(sold_buildings[dt]) {
                $bubble.addClass('active').find('.flats-count').text('Продано');
            } else {
...
*/
            ...
            click: function (el) {
                if(el.at && !el.active) {
                    ...
                    if(spl[0] == plans_current_values.b) {
                        ...
                        pjax.loadPage('/plans/korpus' + spl[0] + '/section' + spl[1] + '/floor' + plans_current_values.f);
                    }
                }
    ...
    function update_quarter_minimap() {
        svg_paper_quarter_minimap.forEach(function(el) {
            ...
            else if(data.living.sections[el.alt] && data.living.sections[el.alt].at) {
                el.bottom.attr({'opacity' : 0.4, 'fill' : '#c8d400'});
                el.attr({'cursor' : 'pointer'});
                el.active = 0;
                el.at = data.living.sections[el.alt].at;
            }
            else {
                el.bottom.attr({'opacity' : 0});
                el.attr({'cursor' : 'default'});
    ...
    function load_apart_details(alt) {
        ...
        if (d && d.st==1) {
            plans_current_values.apart_details.removeClass('not-sale')
                .find('[data-targ="n"] .val').text(d.tn).end()
                .find('[data-targ="rc"] .val').text(d.rc).end()
                .find('[data-targ="sq"] .val').text(d.sq).end()
                .find('[data-targ="tc"] .val').text(Math.round(d.tc / 10000) / 100);
    ...
    frame
        .on('click', '.search_buttons_title', function() {
            ...
            that
                ...
                .siblings('.search_buttons_hidden').transitionToggleHeight(that.hasClass('active'), 500, function() {
                    frame.find('.plans__filters').data('plugin').reinitialise({fix_position:true});
                    frame.find('.plans__filters').data('plugin').scrollToElement(that.parent('.search_buttons_dropdown'), 300);
                });
        })
        .on('click', '.plans__korpus-bubble', function() {
            if($(this).hasClass('active')) {
        ...
        .on('click', '.calculator_btn', function() {
            pjax.loadPage('/mortgage/calculator?id=' + plans_current_values.n);
        })
        .on('click', '.fav_btn', function(){
            var $btn = $(this);
            if($btn.hasClass('active')) {
...
test_json('living', function() {
    ...
    var b_param = 0;
    var filtered_data = {};
    console.log(frame.find('.search_hash').data('building'))
    if (frame.find('.search_hash').data('building')){
        b_param = frame.find('.search_hash').data('building');
        console.log(b_param);
        $.each(data.living.apartments, function(id, apart){
            console.log(apart.b == b_param);
            if(apart.b == b_param) filtered_data[id] = apart;
        });
    }
    if(search_hash ) {
        var split_hash=search_hash.split('&');
    ...
    param_search = frame2.searchInit({
        data: ((Object.keys(filtered_data).length) ? filtered_data : data.living.apartments),
        preview: true,
        ...
        rules: {
            ...
            'perc': function(value, id) {
                var d = data.living.apartments[id];
                var result = Math.round(d.sl * 100 / d.tc) + "%";
...
var slideshow, scroll_controller,
    ...
    .on('click', '.design_gallery .slideshow__bullet', function() {
        slideshow.loadImgNumber($(this).data('targ'));
    })
    .on('click', '.design_gallery .slideshow__arrow', function() {
        console.log($(this).data('targ'));
        slideshow.loadImg($(this).data('targ'));
    })
    .on('click', '.about__scroll-help', function() {
...
page.unloadAnimation=function(targ, next,animationDone){
    var d1=$.Deferred();
    frame.find('.home__col.n4 .home__tile.n7, .home__col.n4 .home__tile.n8, .vr_tour').removeClass('active');
    setTimeout(function(){
        d1.resolve();
    }, 600);
...
setTimeout(function(){
    frame.find('.home__col.n4 .home__tile.n7, .home__col.n4 .home__tile.n8, .vr_tour').addClass('active');
}, 1000);
    /* SLIDESHOW */
    function gallery_start(num){
        if(num == 1){
...
if(!pages_info.previous.type && 0) {
   ...
   // init_content();
	...
	if(targ=="two-level-flat") {(function(){
page.unset=function(){
    frame.off();
    unloadPlugin(slideshow);
};
